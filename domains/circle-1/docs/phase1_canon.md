...
- overlap with existing canon items is acceptable; promotion is justified only if the practice adds a distinct agent-facing decision surface, not merely a new label for an existing one;
...
| Type contracts as documentation | `candidate` | Clearly predictive for agents, but currently still compressed inside broader boundary semantics | `mypy` / `pyright` strictness plus `TypedDict` / `Protocol` / dataclass contracts | pytype, custom schema checks, typed wrapper layers | Medium | Makes agent edits more predictable in Python-heavy repos |
| Test architecture coherence | `candidate` | Important for agent fit, but not yet separated from broader enforcement in session 1 | explicit test topology, reusable fixtures, systematic parametrization | naming conventions, local test templates, fixture linting | Low | Helps agents write native-feeling tests instead of foreign bodies |
| Public API boundary markers | `candidate` | Strong signal, but currently folded into general boundary discipline | `__all__`, `_private`, `py.typed`, experimental/deprecated markers | API docs, import-linter, ADR-backed boundaries | Low | Reduces ambiguity about what agents may depend on or extend |
...
| Deprecation and change-management discipline | `candidate` | Very relevant for future public product posture, but not yet decisive for WEA session-1 self-dogfood | semver + dated deprecation policy + structured changelog | compatibility windows in docs, ADRs, release notes policy | Medium | Important once circle-1 becomes an external-facing OSS product |
